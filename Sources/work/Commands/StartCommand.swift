...
enum StartCommand {
  static func runLocal(_ args: Start, _ cfg: Config) throws {
    ...
    let worktreePath = cfg.worktree.pathTemplate.replacingOccurrences(
      of: "{branch}", with: args.branch)
    ...
    if isDirectory(worktreePath) {
      ...
      if !cfg.worktree.postCreateCommand.isEmpty {
        ...
        _ = try? Shell.run(
          "sh", ["-c", cfg.worktree.postCreateCommand], currentDirectory: worktreePath)
      }
    }
  ...
  static func runRemote(_ args: Start, _ cfg: Config, provider: String) throws {
    ...
    if describe.contract != WorkRemoteContract.version {
      throw WorkError(
        "remote '\(runner.provider)' implements contract \(describe.contract), core expects \(WorkRemoteContract.version)"
      )
    }
    ...
    let promptText = Prompt.build(
      ticket: ticket, branch: args.branch, workspacePaths: describe.workspacePaths, sections: slash)
    ...
    try runner.start(
      branch: args.branch, prompt: promptText, repos: cfg.project.repos,
      params: parseParams(args.param))

    print(
      "Done. Tail the session with: work remote log --provider \(runner.provider) \(args.branch)")
  }
  ...
  static func extractTicketID(_ branch: String) -> String? {
    guard let re = try? NSRegularExpression(pattern: "[a-z]+-[0-9]+", options: [.caseInsensitive])
    else {
      return nil
    }
  ...
  private static func runClaudeHeadless(promptFile: String, logFile: String?, worktree: String)
    throws
  {
