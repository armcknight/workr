...
enum Prompt {
    ...
    static func loadInlinedSections(_ cfg: PromptConfig, commandsDir: String) throws -> [RenderedSection] {
    ...
    static func build(ticket: JSON, branch: String, workspacePaths: [String], sections: [RenderedSection]) -> String {
        var s = ""
        ...
        if let nodes = ticket["comments"]["nodes"].array, !nodes.isEmpty {
            let parts = nodes.map { c -> String in
                ...
                let date = String((c["createdAt"].string ?? "").split(separator: "T", maxSplits: 1, omittingEmptySubsequences: false).first ?? "")
        ...
        s += "\n## Instructions\n\n"
        s += "You are running autonomously inside a remote cloud dev environment — there is no human to confirm with mid-task, so do not stop to ask questions. Make reasonable judgement calls and proceed.\n\n"
    ...
    private static func formatWorkspacePaths(_ paths: [String], branch: String) -> String {
        ...
        case 2:
            ...
            return "The repos you may need are at `\(paths[0])` and `\(paths[1])`, both on branch `\(branch)`.\n"
