...
struct RemoteGroup: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "remote",
        abstract: "Cloud devbox operations via the configured provider plugin (status, log, prompt, test).",
        subcommands: [
            RemoteStatus.self,
            RemoteLog.self,
            RemotePrompt.self,
...
struct RemoteOption: ParsableArguments {
    @Option(name: [.short, .long], help: "Remote provider name (defaults to [remote] provider in config).")
    var provider: String?
}
...
struct RemoteLog: ParsableCommand {
    ...
    func run() throws {
        try RemoteLogCommand.run(Config.load(), provider: opts.provider ?? "", branch: branch, follow: follow)
    }
}
...
struct RemotePrompt: ParsableCommand {
    ...
    func run() throws {
        try RemotePromptCommand.run(Config.load(), provider: opts.provider ?? "", branch: branch, text: text.joined(separator: " "))
    }
}
...
struct RemoteTest: ParsableCommand {
    ...
    func run() throws {
        try RemoteTestCommand.run(Config.load(), provider: opts.provider ?? "", branch: branch, port: port)
    }
}
